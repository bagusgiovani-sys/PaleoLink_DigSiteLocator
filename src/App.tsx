...
import { MapPin, Users, Wrench, Gem, X, Cloud, CloudRain, CloudSnow, Sun, Wind, AlertTriangle, Camera, Satellite, Box, Map, FlaskConical, ShoppingCart, User, DollarSign, TrendingUp } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

type WeatherCondition = 'sunny' | 'rainy' | 'thunderstorm' | 'flood' | 'typhoon';
type SiteType = 'fossil' | 'archaeological';
type WeatherSeverity = 'good' | 'warning' | 'catastrophic';
type MainTab = 'expedition' | 'scientists' | 'marketplace';
...
const DigSiteLocator = () => {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'model'>('details');
  ...
  const digSites: DigSite[] = [
    {
      ...
      has3DModel: true,
      modelPath: "public/assets/mockup_stegosaurus.glb",
    ...
    },
    {
      ...
      has3DModel: true,
      modelPath: "public/assets/mockup_trex.glb",
    ...
    },
    {
      ...
      has3DModel: true,
      modelPath: "public/assets/mockup_velociraptor.glb",
    ...
    },
    {
      ...
      has3DModel: true,
      modelPath: "public/assets/mockup_temple.glb",
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-5xl font-bold text-cyan-400 mb-2 tracking-wider">PALEO LINK</h1>
          <div className="h-1 w-64 mx-auto bg-gradient-to-r from-transparent via-cyan-400 to-transparent"></div>
          <p className="text-cyan-200 mt-3 text-sm tracking-widest">GLOBAL EXCAVATION & PRESERVATION NETWORK</p>
        </div>

        {/* Main Navigation Tabs */}
        <div className="mb-6">
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg border border-cyan-500/30 p-2 flex gap-2">
            <button
              onClick={() => setMainTab('expedition')}
              className={`flex-1 px-6 py-3 rounded-lg font-semibold text-sm tracking-wide transition-all duration-300 flex items-center justify-center gap-2 ${mainTab === 'expedition'
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg font-semibold text-sm tracking-wide transition-all duration-300 flex items-center justify-center gap-2 ${mainTab === 'scientists'
                  ? 'bg-cyan-500/30 text-cyan-400 border border-cyan-500/50 shadow-lg shadow-cyan-500/20'
                  : 'text-gray-400 hover:text-cyan-300 hover:bg-slate-800/50'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg font-semibold text-sm tracking-wide transition-all duration-300 flex items-center justify-center gap-2 ${mainTab === 'marketplace'
                  ? 'bg-cyan-500/30 text-cyan-400 border border-cyan-500/50 shadow-lg shadow-cyan-500/20'
                  : 'text-gray-400 hover:text-cyan-300 hover:bg-slate-800/50'
                }`}
            >
          ...
          {mainTab === 'expedition' ? (
            ...
            >
                                      ...
                                      <div className="bg-slate-800/50 rounded-lg p-4 border border-cyan-500/20">
                                        {site.has3DModel ? (
                                          <>
                                            ...
                                            <div className="bg-slate-900/50 rounded aspect-video mb-3 flex items-center justify-center border border-cyan-500/20">
                                            ...
                                            <button className="w-full px-3 py-2 bg-cyan-500/20 border border-cyan-500/50 rounded text-xs text-cyan-400 hover:bg-cyan-500/30 transition-colors">
                                              Load 3D Model
                                            </button>
                                          </>
                                        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg border border-cyan-500/30 p-4 text-center">
                  <p className="text-cyan-400 text-sm tracking-wide">WEATHER ALERTS</p>
                  <p className="text-3xl font-bold text-white mt-1">{digSites.filter(s => s.weather.alert).length}</p>
                </div>
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg border border-cyan-500/30 p-4 text-center">
                  <p className="text-cyan-400 text-sm tracking-wide">3D MODELS</p>
                  <p className="text-3xl font-bold text-white mt-1">{digSites.filter(s => s.has3DModel).length}</p>
                </div>
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg border border-cyan-500/30 p-4 text-center">
                  <p className="text-cyan-400 text-sm tracking-wide">CRITICAL SITES</p>
            ...
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <div className="bg-slate-800/30 backdrop-blur-sm rounded-lg border-2 border-cyan-500/30 p-6 shadow-2xl shadow-cyan-500/20">
                <h2 className="text-2xl font-bold text-cyan-400 mb-6 tracking-wide">RECRUITMENT</h2>
                  ...
                  {scientists.map((scientist) => (
                    <div key={scientist.id} className="bg-slate-900/60 rounded-lg border border-cyan-500/20 overflow-hidden hover:border-cyan-500/50 transition-all">
                      <div className="p-4">
                        ...
                        <div className="bg-slate-800/50 rounded mb-3 aspect-[3/4] flex items-center justify-center border border-slate-700">
            ...
            >
              <div className="bg-slate-800/30 backdrop-blur-sm rounded-lg border-2 border-cyan-500/30 p-6 shadow-2xl shadow-cyan-500/20">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-cyan-400 tracking-wide">MARKETPLACE</h2>
                    <p className="text-gray-400 text-sm">DINOSAURS › FLYING</p>
                  </div>
                  <button className="px-6 py-2 bg-cyan-600/80 hover:bg-cyan-600 rounded-lg text-white font-semibold flex items-center gap-2 transition-colors">
                    <ShoppingCart className="w-5 h-5" />
                    REFRESH LIST
                    ...
                    <div key={item.id} className="bg-slate-900/60 rounded-lg border border-cyan-500/20 hover:border-cyan-500/50 transition-all">
                      <div className="grid grid-cols-12 gap-4 p-4 items-center">
                        ...
                        <div className="col-span-2 text-center">
                          ...
                          <div className="text-teal-400 text-sm">♀️ {item.amount}</div>
                        </div>
                        ...
                        <div className="col-span-1 flex justify-end">
                          <button className="p-2 bg-cyan-600/80 hover:bg-cyan-600 rounded text-white transition-colors">
                            <ShoppingCart className="w-5 h-5" />
                          </button>
                        </div>
                      </div>
